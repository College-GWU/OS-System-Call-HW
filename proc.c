...
procstat(uint which, struct pstat *ps) 
{
	...
	acquire(&ptable.lock); // acquire spinlock so nothing else can access table during this call

	struct proc * p;
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
		if(which == 0 && p->state != UNUSED) // we have reached the "nth" active process,
			break; // so keep going with this p.

		if(p->state != UNUSED) // if process is not unused (active),
			which--;      	   // decrement which to continue to look for "nth" active process
	}

	// if we ran out of active processes, or the last process is unused return 1
	if(which > 0 || p->state == UNUSED) {
		release(&ptable.lock);
		return 1;
	}
	
	// if process is 'init' or state is "EMBRYO", set ppid to 0 
	if(p->pid == 1 || p->state == EMBRYO)
		ps->ppid = 0;
	else
		ps->ppid = p->parent->pid; // get parent process pid from proc struct
	
	ps->pid = p->pid; // get process pid from proc struct
	strncpy(ps->name, p->name, 16); // get process name from proc struct

	// map enum states to chars
	static char states[] = {[UNUSED] 'U',   [EMBRYO] 'E',  [SLEEPING] 'S',
	                        [RUNNABLE] 'N', [RUNNING] 'R', [ZOMBIE] 'Z'};

	ps->state = states[p->state]; // get process state from proc struct
